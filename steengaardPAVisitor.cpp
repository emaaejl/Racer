...
void SteengaardPAVisitor::recordVarAccess(clang::SourceLocation l, std::pair<unsigned long,AccessType> acc)
{
  ...
  varMod.insert(std::pair<clang::SourceLocation, std::pair<unsigned long,AccessType> >(l,acc));
  varAccInfo.insert(std::pair<clang::SourceLocation, AccInfoType>(l,accInfo));
}

void SteengaardPAVisitor::showVarReadWriteLoc()
{
  //std::multimap<clang::SourceLocation,std::pair<unsigned long,AccessType> >::iterator it=varMod.begin();
  std::multimap<clang::SourceLocation,std::tuple<unsigned long,AccessType,std::string> >::iterator it=varAccInfo.begin();
  ...
  for(;it!=varAccInfo.end();it++)
	...
	{ 
	  ...
	  if(std::get<1>(it->second)==RD) { 
	    ...
	    for(std::set<unsigned long>::iterator i=intsect.begin();i!=intsect.end();i++){
...
std::pair<unsigned long,PtrType> SteengaardPAVisitor::getExprIdandType(clang::Expr *exp)
 {
   unsigned long id;
   ...
   if(clang::DeclRefExpr *dref=dyn_cast<clang::DeclRefExpr>(exp))
     {
       ...
       recordVarAccess(dref->getLocStart(),std::pair<unsigned long,AccessType>(E.first,RD));
       recordVarAccess(dref->getLocEnd(),std::pair<unsigned long,AccessType>(E.first,WR));
     }     
 }
 ...
 void SteengaardPAVisitor::updatePAonBinaryExpr(clang::BinaryOperator *bop)
 {
   ...
   if(opcode==clang::BO_Assign)    
       ...
       else if((lhs_pair.second==PTRZERO) && (rhs_pair.second==PTRONE))
	 {
	   ...
	   recordVarAccess(rhs->getExprLoc(),std::pair<unsigned long,AccessType>(rhs_pair.first,RD));
	   recordVarAccess(lhs->getExprLoc(),std::pair<unsigned long,AccessType>(lhs_pair.first,WR));
	 }
 ...
 std::set<FunctionDecl *> SteengaardPAVisitor::getCallsFromFuncPtr(CallExpr *call)
 {
   ...
   if(clang::DeclRefExpr *ref = clang::dyn_cast<clang::DeclRefExpr>(S))
	 ...
	 if(vdec->getType().getTypePtr()->isFunctionPointerType()){
	   ...
	   for(std::set<unsigned long>::iterator it=pts2func.begin();it!=pts2func.end();it++)
	     {
...
void SteengaardPAVisitor::updatePAOnFuncCall(FunctionDecl *calleeDecl,std::set<Expr*> ActualInArgs,long ActualOutArg)
{
  ...
  if(fsig->params.size()!=ActualInArgs.size()) return;
  if(fsig){
    long int i=0;
...
void SteengaardPAVisitor::updatePABasedOnExpr(unsigned long id, Expr * exp)
{
  ...
  if(expTypePair.second==PTRONE || expTypePair.second==PTRZERO)
    { 
      ...
      recordVarAccess(exp->getExprLoc(),std::pair<unsigned long,AccessType>(expTypePair.first,RD));
    }
...
{ 
  ...
  if(clang::DeclRefExpr *dexpr=dyn_cast<clang::DeclRefExpr>(exp))
      ...
      if(dyn_cast<VarDecl>(dexpr->getDecl()))
	{
	  ...
	  */ 
	  std::pair<unsigned long,PtrType> E=getExprIdandType(exp);
	  recordVarAccess(exp->getExprLoc(),std::pair<unsigned long,AccessType>(E.first,RD));
	  return true;
	} 
    }    
...
bool SteengaardPAVisitor::VisitVarDecl(VarDecl *vDecl)
{
  if(!astContext->getSourceManager().isInSystemHeader(vDecl->getLocStart())) 
	...
	if(exp){
	  ...
	  recordVarAccess(vDecl->getSourceRange().getBegin(),std::pair<unsigned long,AccessType>(key,WR));
	}
      }
    }
  return true;
